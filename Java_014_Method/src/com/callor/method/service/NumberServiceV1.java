 ...
 * 		한개의 클래스에서 method를 중복 선언할수 있다.
 * 단, 조건이 있는데 매개변수가 달라야한다.
 * 		매개변수의 type, 개수, 순서가 다르면 같은 이름의 method를 중복 선언할수 있다.
 * 
 */
public class NumberServiceV1 {
	 ...
	 * 		두개의 변수를 선언하자
	 * 
	 * 두 수의 합을 계산하여 결과가 짝수이면 그 합을 return
	 ...
	 * 
	 * 		단순히 정수만  return 하는 경우는 int type으로 하면 되는데
	 ...
	 * 		addNum(int num1, int num2), addNum(long num1, long num2) 형식으로 method를 선언해야한다
	 * 만약 addNum(실수, 실수) 방식으로 호출하려면
	 * 		addNum(float num1, float num2), addNum(double num1, double num2) 형식으로 method를 선언해야한다
	 * 
