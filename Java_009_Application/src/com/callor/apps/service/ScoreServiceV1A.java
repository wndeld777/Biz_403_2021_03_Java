...
public class ScoreServiceV1A {
	...
	public void makeScores() {
		...
		// 만약 정상적인 범위(0 ~ 100)를 입력하면 반복문을 종료하고, 다음코드가 실행되도록 한다
		while(true) {
	...
	//  method를 호출하여 명령을 실행할 수있도록 하는 Keyword
	//  객체.inputScore() 형식으로 사용 불가
	//  외부에 공개하지 않고, 현재클래스의 코드에서만
	//  호출할 수 있도록 제한하는 Keyword
	
	// Integer : void keyword대신 정수를 선언하는 키워드를 사용하여 method 선언하기
	// method 코드 끝에 반드시 return 명령문이 있어야 한다
	
	// 여기에서 사용된 Integer 키워드는 변수를 선언하는 용도의 키워드가아니고
