...
public class ScoreServiceV5 {
	...
	public void makeScores() {
		...
		for(int i = 0 ; i < intKor.length; i++) {
			...
			for( int j = 0 ; j < subject.length; j++) {
				...
				/*
				 * 입력 업무중에 -1을 입력했을때는 입력업무를 중단하고 main() 으로 되돌아가고 싶다
				 * 이때 break를 사용하면 for() 겹쳐있기 때문에
				 * 원하는대로 코드가 작동되지 않는다
				 * 이럴 경우 return type이 void이지만
				 * return명령을 실행하여 method코드를 중단 할 수 있다.
				 */
				if(scores[j] == -1) {
					return;
				}
	...
	private Integer inputScore(String sub) {
		...
		while(true) {
			...
			System.out.print(">> ");
			score = scan.nextInt();
			//점수 입력을 하는중에 -1을 입력하면 입력하는 업무를 중단하자
			if(score == -1) {
				return -1;
	 ...
	 * 
	 * method나 변수에 static을 부착하면 사용하지 않아도 method코드나, 변수가
	 * 메모리에 상주하게 된다.   
	 */
