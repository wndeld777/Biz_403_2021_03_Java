...
public class ScoreVO {
	...
	private Integer math = 0;  // 추상화단계
	// private : 정보은닉
	
	/*
	 * VO클래스를 모델링하고(추상화) 변수를 private으로 선언하였다(정보은닉)
	 * 은닉된 변수에 접근하기 위하여 getter, setter method를 생성
	 *
	 * getter 와 setter를 생성하고 봤더니 총점(total)과 평균(avg)는 점수가 입력되면 항상
	 * 계산되어야 하는 항목이다
	 * 즉, 외부에서 이 변수에 값을 저장할일이 별로 없을 것 같다
	 * 		외부에서 total, avg 값을 요청하면
	 * 		그때그때 계산하여 return을 하면 될 것같다
	 * 
	 * 1. total, avg 변수를 삭제하고
	 * 2. total, avg 변수의 setter method 를 제거한다
	 * 3. total, avg 변수의 getter method 를 변경한다
	 ...
	 * 
	 *  따라서 ScoreVO클래스에는 자동으로 toString() method 가 있는 것처럼 동작할 것이다.
	 * 
	 *  하지만, Object 클래스에 정의된 toString() method는 사용하는데 의미가 없다
	 * 
	 *  그래서 toString() method를 VO객체의 변수에 담긴 값들을 문자열로 만들어
	 *  return 하는 코드로 다시 작성을 한다
