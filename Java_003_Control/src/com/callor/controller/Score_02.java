...
public class Score_02 {
	...
	public static void main(String[] args) {
		...
		// -2의 31승 ~ 2의 31승 -1까지의 값만 저장할수 있다
		intNum = 100_000_000_0;
		
		// long, Long 으로 선언된 변수에는
		// -2의 63승 ~ 2의 63 -1 까지의 값만 저장
		...
		// intNum1에 저장된 정수를 "실수형으로 변환"하여오차를 무시하고 저장하라
		floatNum1 = intNum1;
		
		// 실수값(float )을 정수형변수에 저장하라 라고 명령을 내리면
		// 자바 컴파일러는 이 코드를 실행하면 실수값의 소수점 이하가 잘릴것이다 라는 것을 안다
		// 또한, 이 코드는 분명 개발자가 실수한 것이다
		// 라고 생각하고, 문법오류로 보여준다
		
		// 개발자가 실제 값의 소수점이하가 잘리는 것을 알고 있으며
		// 		소수점이하 값이 필요 없어서 정수형 변수에 저장 하고 싶다
		 ...
		 * 정수값.0 형식으로 실수로 "자동으로 변환"한 다음 저장을 한다.
		 * 매우 큰값은 오차가 발생할 수 있지만
		 * 일반적으로 사용하는 숫자는 오차없이 실수형 변수에 저장이 가능하다
		 */
		floatNum1 = intNum1;
		/*
		 * 강제형 변환
		 ...
		 * 
		 * 만약 개발자가(작성자가) 오차를 무시하고 강제로 실수값을 정수형에 변환하여 저장하고자
		 * 한다면 "강제로 변환하는" 코드를 첨가하면 된다
